...
func main() {
	...
	if api, err = cmd.NewAPI(apiConf, log.NewZapTracedLogger(sLog.With("service", "api"))); err != nil {
		traced.Errorw(domain.RuntimeTraceID, "failed to create api app: %s", err)
		os.Exit(1)
	}
	if web, err = cmd.NewWeb(webConf, log.NewZapTracedLogger(sLog.With("service", "web"))); err != nil {
		traced.Errorw(domain.RuntimeTraceID, "failed to create web app: %s", err)
		os.Exit(1)
	}
	...
	go func() {
		...
		if err := api.Run(runCtx); err != nil {
			traced.Errorw(domain.RuntimeTraceID, "API run failed", err)
		}
	}()
	go func() {
		defer wg.Done()
		...
		if err := web.Run(runCtx); err != nil {
			traced.Errorw(domain.RuntimeTraceID, "Web run failed", err)
		}
	}()

	wg.Wait()
	traced.Infow(domain.RuntimeTraceID, "All apps stopped")
}
